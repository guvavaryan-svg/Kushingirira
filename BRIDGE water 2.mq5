...
input bool   UseNewsFilter       = true;      // Enable MQL5 Calendar News Filter
input int    MinutesBeforeNews   = 30;        // Pause x mins before high impact news
input int    MinutesAfterNews    = 30;        // Pause x mins after high impact news
...
void UpdateDashboard()
{
   ...
   CreateDashboardLabel("Title", "--- NEVER GIVE UP EA V6.1 ---", 25, 20, clrGold, 10, true);
...
{ 
   ...
   int total = CalendarValueHistory(values, from_time, to_time, NULL, curr);
   if(total > 0)
   {
...
bool CheckMTFTrend(int signal_type)
{
   ...
   if(CopyBuffer(handleHTFFast, 0, 1, 1, htfFast) <= 0 || CopyBuffer(handleHTFSlow, 0, 1, 1, htfSlow) <= 0)
      return false;
...
bool CheckSignals(int &signal)
{
   ...
   // Reading Closed Bars [1] and [2] for absolute precision (No Repainting)
   ...
   // Filter validations based on closed candle [1]
