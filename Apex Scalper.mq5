//+------------------------------------------------------------------+
//| Never give Up.mq5                                                |
//| Strategy: EURUSD Scalper, Fast Signal & Basket Breakeven Engine  |
//| Version: 10.5 EURUSD MASTER EDITION (PRO COMMERCIAL RELEASE)     |
//+------------------------------------------------------------------+
#property strict
...
input bool   CloseTradesBeforeNews = false;   // Emergency exit open basket before high news
...
input bool   UseGridSystem       = true;      
input double GridStepPips        = 15.0;      
input int    MaxGridOrders       = 3;         
input double GridLotMultiplier   = 1.0;       

input group "=== INDICATOR SIGNAL FILTERS ==="
input int    FastMA              = 5;         
input int    SlowMA              = 20;        
input int    RSIPeriod           = 14;
input double RSIOverbought       = 55.0;      // Relaxed for faster signal execution
input double RSIOversold         = 45.0;      // Relaxed for faster signal execution

input group "=== BASKET PROFIT, BREAKEVEN & TRAILING ==="
input double TargetBasketProfit  = 0.50;      // Close ALL trades when Floating Profit hits ($)
input double MaxBasketLossDollars= 2.00;      // Emergency Basket Loss Cap ($)
input bool   UseBasketBreakeven  = true;      // Move SL to Entry when near target
input double BasketBreakevenTrigger = 0.40;   // Trigger Breakeven at ($0.40 profit)
input bool   UseBasketTrailing   = true;      
input double BasketTrailStart    = 0.30;      
input double BasketTrailStep     = 0.20;      // Adequate breathing space ($0.20)
...
int OnInit()
{
   ...
   Print("NEVER GIVE UP V10.5 MASTER EDITION (PRO COMMERCIAL RELEASE) LOADED.");
...
void UpdateDashboard()
{
   ...
   CreateDashboardLabel("Title", "--- EURUSD MASTER V10.5 ---", 25, 20, clrGold, 10, true);
...
//+------------------------------------------------------------------+
//| Native MT5 Economic Calendar News Filter                         |
//+------------------------------------------------------------------+
   ...
   if(GetTotalBasketProfit() >= BasketBreakevenTrigger)
   {
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         if(sym == _Symbol && PositionGetInteger(POSITION_MAGIC) == MagicNumber)
         {
            ...
            // Expanded buffer to 5.0 pips for Razor/ECN spread safety
      ...
      bot_status_msg = "BASKET BREAKEVEN SET (+5 Pips)";
   }
}
...
void ExecuteInstantTrade()
{
   ...
   if(trade.Buy(lot, _Symbol, ask, sl, tp, "EURUSD_Instant_Buy"))
   {
      daily_trade_count++;
...
void ManageBasketCloseAndTrailing()
{
   ...
   {
      CloseAllPositions();
      bot_status_msg = "BASKET STOPLOSS HIT";
      return;
   }
...
void ProcessGridEntries()
{
   ...
   if(buy_count > 0 && buy_count < MaxGridOrders)
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         if(PositionGetSymbol(i) == _Symbol && PositionGetInteger(POSITION_MAGIC) == MagicNumber && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
         {
            ...
            if(p < lowest_buy_price) lowest_buy_price = p;
         }
      }

      if(current_ask <= lowest_buy_price - (GridStepPips * pip_size))
      {
         double next_lot = CleanAndNormalizeLot(FixedLotSize * MathPow(GridLotMultiplier, buy_count));
         double sl = UseHardSLTP ? NormalizeDouble(current_ask - (StopLossPips * pip_size), _Digits) : 0;
         double tp = UseHardSLTP ? NormalizeDouble(current_ask + (TakeProfitPips * pip_size), _Digits) : 0;

         if(trade.Buy(next_lot, _Symbol, current_ask, sl, tp, "Grid_Buy_Layer_" + IntegerToString(buy_count)))
         {
            daily_trade_count++;
   ...
   if(sell_count > 0 && sell_count < MaxGridOrders)
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         {
            double p = PositionGetDouble(POSITION_PRICE_OPEN);
            if(p > highest_sell_price) highest_sell_price = p;
         }
      }

      if(current_bid >= highest_sell_price + (GridStepPips * pip_size))
      {
         double next_lot = CleanAndNormalizeLot(FixedLotSize * MathPow(GridLotMultiplier, sell_count));
         double sl = UseHardSLTP ? NormalizeDouble(current_bid + (StopLossPips * pip_size), _Digits) : 0;
         double tp = UseHardSLTP ? NormalizeDouble(current_bid - (TakeProfitPips * pip_size), _Digits) : 0;

         if(trade.Sell(next_lot, _Symbol, current_bid, sl, tp, "Grid_Sell_Layer_" + IntegerToString(sell_count)))
         {
            daily_trade_count++;
...
void OpenInitialTrade(int signal)
{
   ...
   if(signal == 1)
   {
      ...
      if(trade.Buy(lot, _Symbol, ask, sl, tp, "EURUSD_Buy"))
      {
         daily_trade_count++;
   ...
   else if(signal == -1)
   {
      ...
      if(trade.Sell(lot, _Symbol, bid, sl, tp, "EURUSD_Sell"))
      {
         daily_trade_count++;
...
void OnTick()
{
   ...
   // 3. Basket Risk Checks & Trailing Logic
   CheckEquityProtection();
   ManageBasketCloseAndTrailing();
   
   // 4. Process Grid Layers (Guarded by Circuit Breakers above)
   ProcessGridEntries();

   // 5. Check Signal for New Initial Position
   int signal;
