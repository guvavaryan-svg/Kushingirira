//+------------------------------------------------------------------+
//| Never give Up.mq5                                               |
//| Strategy: EMA + RSI + SAR + MACD + DIP/RIP + MARTINGALE         |
//| Version: 6.6 Deriv Protected & Enhanced (Gold XAUUSD Optimized) |
//+------------------------------------------------------------------+
#property strict
...
//--- STRATEGY INPUTS (OPTIMIZED FOR XAUUSD / GOLD)
...
//--- PARABOLIC SAR & MACD INPUTS
input bool   UseSARFilter        = true;      // Use Parabolic SAR Filter?
...
//--- GOLD (XAUUSD) ADJUSTED SL / TP & SPREAD
input double StopLossPips        = 150.0;     // SL in Pips (150 Pips for Gold safety)
input double TakeProfitPips      = 300.0;     // TP in Pips (300 Pips for Gold targets)
input double BreakEven           = 100.0;     // Break Even trigger in Pips
input double TrailStart          = 120.0;     // Trailing Stop start in Pips
input double TrailStep           = 30.0;      // Trailing Stop step in Pips
input int    MaxSpreadPoints     = 500;       // Spread limit (50 Pips limit for Gold)

//--- PARTIAL TAKE PROFIT
input bool   UsePartialClose     = true;      // Close half volume at Partial TP?
input double PartialTPTriggerPips= 150.0;     // Pips to trigger Partial Close

//--- MULTI-TIMEFRAME TREND FILTER
input bool   UseMTFTrendFilter   = false;     // Set to false for instant M1 signals
input ENUM_TIMEFRAMES HTF_Period = PERIOD_M1; // M1 Timeframe
...
input double MartingaleMultiplier= 1.3;      // Deriv Safety Level: 1.3
input int    MaxMartingaleSteps  = 3;

//--- SIDEWAYS FILTER & EMERGENCY EXIT
input bool   UseSidewaysFilter   = false;     // Set to false for aggressive entry
...
input int    MaxDailyTrades      = 10;        // Max trades allowed per day (0 = Unlimited)
input bool   CloseFridayWeekend  = true;      // Auto-close trades on Friday evening
input int    FridayCloseHour     = 21;        // Hour to close trades on Friday (24h format)

//--- PROTECTION & MANAGEMENT INPUTS
input int    MagicNumber         = 12347; 
input bool   TradeOnlyOne        = true;
input double MaxDailyLossPercent = 5.0;       // Reduced to 5% for equity safety
input double MaxTotalDrawdownPercent = 8.0;   // Reduced to 8% for drawdown limit
input bool   CloseAllOnHalt      = true;
input bool   UseTradingHours     = false;     // Set to false so it trades 24/7
input int    StartHour           = 0;             
input int    EndHour             = 24;              
input int    CooldownMinutes     = 1;         // Cooldown in minutes

//--- CALENDAR NEWS FILTER
input bool   UseNewsFilter       = true;      // Set false for Synthetic Indices
...
int OnInit()
{
   ...
   handleSAR    = iSAR(_Symbol, PERIOD_CURRENT, SARStep, SARMaximum);
   ...
   if(handleFastMA == INVALID_HANDLE || handleSlowMA == INVALID_HANDLE || 
      ...
      handleADX == INVALID_HANDLE || handleSAR == INVALID_HANDLE || handleMACD == INVALID_HANDLE)
   {
   ...
   Print("NEVER GIVE UP V6.6 DERIV SAFE (GOLD READY) - Symbol: ", _Symbol, " | Magic: ", MagicNumber);
...
void UpdateDashboard()
{
   ...
   CreateDashboardLabel("Title", "--- NEVER GIVE UP EA V6.6 ---", 25, 20, clrGold, 10, true);
...
{ 
   ...
   int total_base = CalendarValueHistory(values, from_time, to_time, NULL, base_curr);
   for(int i = 0; i < total_base; i++)
   {
      ...
      if(CalendarEventById(values[i].event_id, event))
      {
         if((FilterHighImpact && event.importance == CALENDAR_IMPORTANCE_HIGH) ||
            (FilterMediumImpact && event.importance == CALENDAR_IMPORTANCE_MODERATE) ||
            (FilterLowImpact && event.importance == CALENDAR_IMPORTANCE_LOW))
         {
            bot_status_msg = "PAUSED (NEWS: " + base_curr + ")";
            return true;
         }
      }
   }

   int total_profit = CalendarValueHistory(values, from_time, to_time, NULL, profit_curr);
   for(int i = 0; i < total_profit; i++)
   {
      ...
      if(CalendarEventById(values[i].event_id, event))
      {
         if((FilterHighImpact && event.importance == CALENDAR_IMPORTANCE_HIGH) ||
            ...
            (FilterLowImpact && event.importance == CALENDAR_IMPORTANCE_LOW))
         {
            bot_status_msg = "PAUSED (NEWS: " + profit_curr + ")";
            return true;
         }
      }
   }
...
bool CheckSignals(int &signal)
{
   ...
   double fastMA[], slowMA[], rsi[], sar[], macdMain[], macdSignal[];
   ...
   if(CopyBuffer(handleFastMA, 0, 1, 2, fastMA) < 2 ||
      ...
      CopyBuffer(handleRSI, 0, 1, 1, rsi) < 1 ||
      CopyBuffer(handleSAR, 0, 1, 1, sar) < 1 ||
      CopyBuffer(handleMACD, 0, 1, 1, macdMain) < 1 ||
      CopyBuffer(handleMACD, 1, 1, 1, macdSignal) < 1) return false;

   double close_price = iClose(_Symbol, PERIOD_CURRENT, 1);

   bool sar_buy  = !UseSARFilter || (close_price > sar[0]);
   bool sar_sell = !UseSARFilter || (close_price < sar[0]);
   bool macd_buy = !UseMACDFilter || (macdMain[0] > macdSignal[0]);
   bool macd_sell= !UseMACDFilter || (macdMain[0] < macdSignal[0]);

   // Core Trend Crossover logic
   if(fastMA[1] <= slowMA[1] && fastMA[0] > slowMA[0] && rsi[0] > 50 && sar_buy && macd_buy) 
      signal = 1;

   if(fastMA[1] >= slowMA[1] && fastMA[0] < slowMA[0] && rsi[0] < 50 && sar_sell && macd_sell) 
      signal = -1;
   ...
   if(UseDipRip)
   {
      ...
      if(CopyBuffer(handleATR, 0, 1, 1, atr) > 0)
      {
         ...
         if(close_price <= dip_level && rsi[0] < RSIDip && sar_buy && macd_buy) signal = 1;
         if(close_price >= rip_level && rsi[0] > RSIRip && sar_sell && macd_sell) signal = -1;
      }
   }
...
void OpenTrade(int signal)
{
   ...
   // DERIV STOPS LEVEL CHECK
   double min_stop_level = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL) * _Point;
   if(sl_distance < min_stop_level) sl_distance = min_stop_level + (100 * _Point);
   if(tp_distance < min_stop_level) tp_distance = min_stop_level + (100 * _Point);
