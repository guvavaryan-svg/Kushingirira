...
// CHECK LAST TRADE TO MANAGE MARTINGALE STEP CORRECTLY ACCORDING TO SL/TP
...
bool EquityProtector()
{
   ...
   Comment("Running | Martingale Step: ", martingale_step, " | Daily Loss: ", DoubleToString(daily_loss, 2), "%");
   return true;
}
...
void OpenTrade(int signal)
{
   ...
   if(signal == 1)
   {
      ...
      if(trade.Buy(lot, _Symbol, price, sl, tp, "FX_MART_Buy_S" + IntegerToString(martingale_step)))
      {
         ...
         Print("BUY OPENED - Lot: ", lot, " Step: ", martingale_step, " SL: ", sl, " TP: ", tp);
      }
   }
   else if(signal == -1)
   {
      ...
      {
         last_trade_time = TimeCurrent();
         Print("SELL OPENED - Lot: ", lot, " Step: ", martingale_step, " SL: ", sl, " TP: ", tp);
      }
   }
}
