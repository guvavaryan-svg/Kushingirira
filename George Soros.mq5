...
//| Version: 6.1 Debug & All-Days Enabled                           |
//+------------------------------------------------------------------+
#property strict
...
input bool   EmergencyExitSideways = true;    // Emergency Exit open trades on Sideways detected?
input int    ADXPeriod           = 14;
input double ADXThreshold        = 20.0;
input int    ATRPeriod           = 14;
input double MinATR              = 0.00005;      

//--- DAY FILTER INPUTS (NOW INCLUDES MONDAY & FRIDAY)
...
input bool   UseNewsFilter       = false;     // Turned OFF by default for smooth testing
...
int OnInit()
{
   ...
   Print("PRO BOT V6.1 READY - Symbol: ", _Symbol, " | Magic: ", MagicNumber);
...
void UpdateDashboard()
{
   ...
   CreateDashboardLabel("Title", "--- NEVER GIVE UP EA V6 ---", 25, 20, clrGold, 10, true);
...
bool IsMarketTrending()
{
   ...
   if(adx[0] < ADXThreshold || atr[0] < MinATR)
   {
      ...
      if(EmergencyExitSideways && PositionsTotal() > 0)
      {
         CloseAllPositions();
...
//--- DAY OF WEEK FILTER (UPDATED FOR MONDAY - FRIDAY) ---
bool CheckAllowedDays()
{
   ...
   bool allowed = false;
   if(dt.day_of_week == 1 && TradeMonday)    allowed = true; // Monday
   if(dt.day_of_week == 2 && TradeTuesday)   allowed = true; // Tuesday
   if(dt.day_of_week == 3 && TradeWednesday) allowed = true; // Wednesday
   if(dt.day_of_week == 4 && TradeThursday)  allowed = true; // Thursday
   if(dt.day_of_week == 5 && TradeFriday)    allowed = true; // Friday
...
//--- MQL5 ECONOMIC CALENDAR FILTER ---
bool IsNewsTime()
{ 
   if(!UseNewsFilter) return false; 
   
   datetime from_time = TimeCurrent() - (MinutesBeforeNews * 60);
   datetime to_time   = TimeCurrent() + (MinutesAfterNews * 60);
   
   MqlCalendarValue values[];
   string curr = SymbolInfoString(_Symbol, SYMBOL_CURRENCY_BASE);
   
   if(CalendarValueHistory(values, from_time, to_time, NULL, curr) > 0)
   {
      for(int i = 0; i < ArraySize(values); i++)
      {
         ...
         if(CalendarEventById(values[i].event_id, event))
         {
            ...
            {
               bot_status_msg = "PAUSED (HIGH NEWS)";
               return true;
            }
         }
      }
...
bool CheckMTFTrend(int signal_type)
{
   ...
   Print("DEBUG: MTF Trend Filter failed signal match. Signal: ", signal_type);
   return false;
}
...
bool CheckSignals(int &signal)
{
   ...
   if(CopyBuffer(handleFastMA, 0, 0, 2, fastMA) < 2 ||
      ...
      CopyBuffer(handleSAR, 0, 0, 1, sar) < 1 ||
      CopyBuffer(handleMACD, 0, 0, 1, macdMain) < 1 ||
      CopyBuffer(handleMACD, 1, 0, 1, macdSignal) < 1) return false;
   ...
   bool macd_buy = !UseMACDFilter || (macdMain[0] > macdSignal[0] && macdMain[0] > 0);
   bool macd_sell= !UseMACDFilter || (macdMain[0] < macdSignal[0] && macdMain[0] < 0);
   ...
   if(UseDipRip)
   {
      ...
      if(CopyBuffer(handleATR, 0, 0, 1, atr) > 0)
      {
         ...
         if(close_price <= dip_level && rsi[0] < RSIDip && sar_buy && macd_buy && CheckMTFTrend(1)) signal = 1;
         if(close_price >= rip_level && rsi[0] > RSIRip && sar_sell && macd_sell && CheckMTFTrend(-1)) signal = -1;
      }
   }
...
void OpenTrade(int signal)
{
   ...
   {
      Print("DEBUG: Calculated Lot size too small: ", lot);
      return;
   }
   ...
   if(signal == 1)
   {
      ...
      Print("DEBUG: Attempting BUY execution at ", price);
      if(trade.Buy(lot, _Symbol, price, sl, tp, "NGU_Buy_S" + IntegerToString(martingale_step)))
      {
         last_trade_time = TimeCurrent();
         Print("BUY OPENED SUCCESSFULLY - Lot: ", lot, " Step: ", martingale_step);
      }
      else
      {
         Print("BUY EXECUTION FAILED! Error code: ", GetLastError());
      }
   }
   else if(signal == -1)
   {
      ...
      Print("DEBUG: Attempting SELL execution at ", price);
      if(trade.Sell(lot, _Symbol, price, sl, tp, "NGU_Sell_S" + IntegerToString(martingale_step)))
      {
         last_trade_time = TimeCurrent();
         Print("SELL OPENED SUCCESSFULLY - Lot: ", lot, " Step: ", martingale_step);
      }
      else
      {
         Print("SELL EXECUTION FAILED! Error code: ", GetLastError());
      }
   }
}
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(ticket > 0 && PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == MagicNumber)
      {
         ...
         if(TrailStart > 0 && profit_pips >= TrailStart)
         {
            ...
            { 
               new_sl = current_price + TrailStep * pip_size; 
               if(sl == 0 || new_sl - (pip_size * 0.5) > sl) 
                  trade.PositionModify(ticket, new_sl, tp);
            }
         }
      }
...
void OnTick()
{
   ...
   int signal;
   if(CheckSignals(signal)) 
   {
      Print("DEBUG: Trade signal generated! Signal value: ", signal);
      OpenTrade(signal);
   }
