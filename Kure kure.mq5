...
input int StartHour = 7;             // London Open
...
// FIX: Strictly normalizes lot size to broker specifications & rounds off floats
double CalculateLotSize(double sl_pips)
{
   ...
   // Strict Lot Normalization to prevent 'Invalid volume' errors
   if(lot_step > 0)
      lot = MathFloor(lot / lot_step) * lot_step;
      
   lot = NormalizeDouble(lot, 2); // Force 2 decimal places maximum (e.g. 0.01)
