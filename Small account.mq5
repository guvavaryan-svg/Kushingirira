...
//| Version: 10.3 EURUSD MASTER EDITION                              |
//+------------------------------------------------------------------+
#property strict
...
int OnInit()
{
   ...
   Print("NEVER GIVE UP V10.3 MASTER EDITION LOADED SUCCESSFULLY.");
...
void UpdateDashboard()
{
   ...
   CreateDashboardLabel("Title", "--- EURUSD MASTER V10.3 ---", 25, 20, clrGold, 10, true);
   ...
   if(GetTotalBasketProfit() >= BasketBreakevenTrigger)
   {
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         if(sym == _Symbol && PositionGetInteger(POSITION_MAGIC) == MagicNumber)
         {
            ...
            double be_sl = (type == POSITION_TYPE_BUY) ? open_p + (2.0 * pip_size) : open_p - (2.0 * pip_size);
      ...
      bot_status_msg = "BASKET BREAKEVEN SET";
   }
}
...
void OnTick()
{
   ...
   if(CountOurPositions() == 0 && CheckTradingHours()) bot_status_msg = "ACTIVE (SCANNING)";
   UpdateDashboard();
}
